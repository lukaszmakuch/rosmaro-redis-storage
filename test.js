...
describe("Redis storage for Rosmaro", function () {
  ...
  describe("without ttl", function () {
    ...
    it("doesn't call pexpire", async function () {
      const setting = storage.set_data({a: 123})
      redis_client.tick()
      await setting

      const getting = storage.get_data()
      redis_client.tick()
      await getting
      assert.deepEqual(redis_client.pexpire_log, [])
    })
  })
  ...
  describe("with ttl", function () {
    ...
    beforeEach(function () {
      ...
      key = "abc"
      const ttl = 123
      storage = make_rosmaro_redis_storage({ redis_client, key, ttl })
    })

    it("calls pexpire on set", async function () {
      const setting = storage.set_data({a: 123})
      redis_client.tick()
      redis_client.tick()
      await setting

      assert.deepEqual(redis_client.pexpire_log, [["abc", 123]])
    })

    it("calls pexpire on get", async function () {
      const getting = storage.get_data()
      redis_client.tick()
      redis_client.tick()
      await getting
    ...
    it("is transparent for errors thrown by pexpire", async function () {
      const expected_err = new Error('expected')
      let thrown

      try {
        const operation = storage.set_data({a: 123})
        redis_client.tick()
        redis_client.fail_with(expected_err)
        redis_client.tick()
        await operation
      } catch (err) { thrown = err }

      assert.deepEqual(expected_err, thrown)
    })
